    ...
    //if(array != undefined){
        ...
        for(let i =0; i < tags.length; i++) {
            ...
            for(let id=0; id<array.length; id++){
                if(array[id] != tags[i].textContent){
                    result.push(array[id]);
                }
            }
            array = result;
            //array = array.filter(arg =>  arg != tags[i].textContent);
        }
        let id = 0;
...
inputFieldA.addEventListener('focus', function (e) {
...
inputFieldI.addEventListener('focus', function (e) {
